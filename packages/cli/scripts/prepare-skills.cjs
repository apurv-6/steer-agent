...
if (!fs.existsSync(SKILLS_SRC)) {
  console.error('prepare: .claude/skills/ not found at', SKILLS_SRC);
} else {
  ...
  console.error('prepare: copied', count, 'skills to packages/cli/skills/');
}
...
if (!fs.existsSync(EXT_SRC)) {
  console.error('prepare: packages/cursor-extension/ not found — skipping vsix copy');
} else {
  const vsixFiles = fs.readdirSync(EXT_SRC).filter(f => f.endsWith('.vsix')).sort().reverse();
  if (vsixFiles.length === 0) {
    console.error('prepare: no .vsix files in packages/cursor-extension/ — skipping');
  } else {
    ...
    console.error('prepare: copied', vsixFiles[0], 'to packages/cli/extension/');
  }
}
